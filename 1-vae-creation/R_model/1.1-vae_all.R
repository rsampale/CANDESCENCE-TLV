...
vae %>% compile(optimizer = optimizer_adam(learning_rate = LR), loss = vae_loss3)  # better than rmsprop
...
# second encoder model which encodes all the way to z, for use in standalone_decoder
z_encoder <- keras_model(x,z) # doesn't make a difference

## build a digit generator that can sample from the learned distribution ----> THIS SHOULD BE AFTER THE TRAINING???
...
standalone_decoder <- keras_model(decoder_input, d_x_decoded_mean_squash_crp)

#### Model Fitting ####


with(tensorflow::tf$device('GPU:7'), {
  vae %>% fit(
    x_train, x_train, 
    shuffle = TRUE,
    epochs = eps,
...
if (visualization) {
  ...
  decoded_imgs = standalone_decoder %>% predict(encoded_stuff)
